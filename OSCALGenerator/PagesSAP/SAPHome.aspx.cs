...
namespace OSCALGenerator.PagesSAP
{
    public partial class SAPHome : BasePage
    {
        ...
        protected new void Page_Load(object sender, EventArgs e)
        {
            try
            {
                ...
                OscalSchemaPath = string.Format(@"{0}\Templates\{1}", appPath, SAPschema);
                ...
                if (Cache["username"] != null)
                {
                    ...
                    if (Cache["SystemId"] != null)
                    {
                        ...
                        UserDocuments = GetSystemDocFullName(SystemID, 6);
                        DocInfos = GetSystemDocInfo(SystemID, 6);
                    }
        ...
        void CorrectXMLGenerateXMLDoc()
        {
           ...
           // Xwriter.WriteRaw("<?xml-stylesheet type=\"text/xsl\" href=\"FedRAMP-compliance-worksheet.xsl\"?>\n");
            ...
            Xwriter.WriteStartElement("assessment-plan", @"http://csrc.nist.gov/ns/oscal/1.0");
          ...
          //  Xwriter.WriteAttributeString("schemaLocation", "xsi", @"http://csrc.nist.gov/ns/oscal/1.0/oscal_ssp_schema.xsd");
            ...
            message = string.Format("Rendering the OSCAL Document {0}:  sucessfully completed Import from SSP ... ", FileName);
            percent = 23;
            PrintProgressBar(message, percent - wordPercent);
        ...
        private void WriteResponsibleParties(List<ResponsibleParty> P)
        {
            foreach (var y in P)
            {
        ...
        private void WriteProps(List<Prop> props)
        {
            if (props != null)
        ...
        private void WriteAnns( List<Annotation> anns)
        {
            if (anns != null)
        ...
        private void WriteLinks(List<Link> links)
        {
            if (links != null)
        ...
        private void XMLImportSSP()
        {
            Xwriter.WriteStartElement("import-ssp");
...
//        X.WriteEndElement();
        ...
        protected void StartButton_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                if (FileUpload1.HasFile)
                {
                    ...
                    var shortName = string.Format("{0} {1}", nameOnly, "SAP");
                    var fullName = string.Format("{0} {1}", nameOnly, "Security Assessment Plan");

                    var userDocs = GetSystemDocInfo(SystemID,6);
                    int rank = -1;
                    ...
                    if (rank >= 0)
                        ...
                        DOID = AddDoc(shortName, fullName, SystemID, 6, 1, SystemID);

                    Cache.Insert("doid", DOID);
                    Cache.Insert("userId", UserName);
                    Cache.Insert("selectedFile", fileName);
                    ImportToDBSAP(XMLNamespace, OscalSchemaPath, FileToConvertPath, DOID, SystemID);
        ...
        void XwriteAddress(DocAddress party)
        {
        ...
        private void XMLMetadata()
        {
            ...
            foreach (var role in roles)
            {
                ...
                if (role.Props != null)
                    foreach (var prop in role.Props)
                    {
                        Xwriter.WriteStartElement("prop");
                        Xwriter.WriteAttributeString("name", prop.Name);
                        Xwriter.WriteAttributeString("id", prop.ID);
                ...
                if (role.Annotations != null)
                    foreach (var ann in role.Annotations)
                    {
                        Xwriter.WriteStartElement("annotation");
                        Xwriter.WriteAttributeString("name", ann.Name);
                        Xwriter.WriteAttributeString("id", ann.ID);
        ...
        protected void OSCALButton_Click(object sender, EventArgs e)
        {
            try
            {
               // var guid = Guid.NewGuid().ToString(); 
                var ErrorFilePath = string.Format(@"{0}Uploads\{1}ValidationErrors.txt", AppPath, "SAP" );
                ...
                string xmlSchemaPath = HttpContext.Current.Server.MapPath(string.Format(@"~\Templates\{0}", SAPschema));
            ...
            catch (Exception ex)
            {
                CollapseDiv("Form");
                StatusLabel.ForeColor = System.Drawing.Color.Red;
                var path = string.Format(@"{0}Uploads\{1}ValidationErrors.txt", AppPath, "SAP"); ;
